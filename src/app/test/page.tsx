      ...
      setTimeout(() => {
        ...
        if (!isSwitchOn) {
          eventSource = new EventSource(
            `${API_URL}/api/v1/build-docker-image?parentDirectory=projects&branchName=${valueBr}&repoUrl=${valueRep}&imageName=${valueIma}&isPrivate=${true}&projectType=${valuePro.values().next().value}`,
            {
          ...
          eventSource = new EventSource(
            `${API_URL}/api/v1/build-docker-image?parentDirectory=projects&branchName=${valueBr}&repoUrl=${valueRep}&imageName=${valueIma}&isPrivate=${false}&username=${valueUsername}&token=${valueToken}&projectType=${valuePro.values().next().value}`,
            {
              headers: { "Content-Type": "text/event-stream" },
            },
          );
